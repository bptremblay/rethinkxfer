      ...
      var func = function (tableName) {
        ...
        r.db(source).table(tableName).run(conn, {
          ...
          cursor.toArray(function (err, result) {
            ...
            console.log('createJSONFromTables: Created "' + jsonFileName + '".');
            completionCount++;
            if (completionCount === res.length) {
              cb(done);
            }
...
function createTablesFromJSON(res, r, conn, cb) {
  ...
  for (var t = 0; t < res.length; t++) {
    ...
    var func = function (tableName) {
      console.log('createTables: Creating "' + tableName + '".');
      console.log('createTables: dropping "' + tableName + '".');
      r.db(destination).tableDrop(tableName).run(conn, function () {
        // might fail... does it matter?
        console.log('createTables: dropped, so we will create a new table "' + tableName + '".');
        r.db(destination).tableCreate(tableName).run(conn, function (err, newTable) {
          if (err) {
          ...
          try {
            ...
            console.log('createTables: JSON.parse "' + newTable.config_changes[0].new_val.name + '": ', parseErr.message);
          }
          if (newJSON.length > MAX_TABLE_LENGTH) {
            remainingBuffer[tableName] = [];
            remainingBuffer[tableName] = newJSON.slice(MAX_TABLE_LENGTH);
            console.warn('Truncated a huge table!!!');
            newJSON = newJSON.slice(0, MAX_TABLE_LENGTH);
          }
          console.log('Inserting ' + newJSON.length + ' records for table "' + tableName + '".');
